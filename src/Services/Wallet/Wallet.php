...
use Api\Services\Exception\CredentialsError;
use Api\Services\Exception\ParameterError;
...
class Wallet {
    ...
    public function getBalance() {
        ...
        return \Api\Services\Conversion\Conversion::BTC_int2str($json['balance']);
    }
    ...
    public function send($to_address, $amount, $from_address=null, $fee=null) {
        ...
        $params = array(
            'to'=>$to_address,
            'amount'=>\Api\Services\Conversion\Conversion::BTC_float2int($amount)
        );
        if(!is_null($from_address))
            $params['from'] = $from_address;
        if(!is_null($fee))
            $params['fee'] = \Api\Services\Conversion\Conversion::BTC_float2int($fee);
    ...
    public function sendMany($recipients, $from_address=null, $fee=null) {
        ...
        foreach ($recipients as $address => $amount) {
            $R[] = '"' . $address . '":' . \Api\Services\Conversion\Conversion::BTC_float2int($amount);
        }
        $json = '{' . implode(',', $R) . '}';
