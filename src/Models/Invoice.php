    ...
    public function getAllInvoiceDBTransactions($user_name = '') {
        try {
            ...
            if ($user_name == '') {
                ...
                $stmt = $this->pdo->prepare("SELECT i.id,i.Paydate,i.Invoiceid,i.Purpose,i.Btcaddress,i.Amount,i.Btcamount,(CASE pcl.status WHEN '1' THEN 'Unpaid' WHEN '2' THEN 'Paid' ELSE 'N/A' END ) AS Status,pcl.response,i.status,i.created_at,i.Username, (CASE WHEN i.Purpose = 'Registration' THEN 'N/A' ELSE IFNULL(DATE_FORMAT(DATE_ADD(i.created_at, INTERVAL 365 DAY),'%d-%m-%Y'),'N/A') END ) as expiry_date,pcl.id as log_id FROM invoice AS i LEFT JOIN payment_callback_log AS pcl ON pcl.invoice_id=i.Invoiceid  having pcl.id = (select id from payment_callback_log where invoice_id = i.Invoiceid  order by id desc limit 1 ) order by log_id desc ");
                $stmt->execute();
            } else {
